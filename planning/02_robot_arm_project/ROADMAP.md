# SO-101 Learning Roadmap

> Status: paused. This roadmap is retained for the future standalone robot-arm project. The active repository plan is [../README.md](../README.md). Common geometry, kinematics, optimization and research skills are tracked in [../00_common_foundations/README.md](../00_common_foundations/README.md); when this project resumes, reuse completed evidence instead of repeating it.

这是本项目恢复后的学习执行入口，也是持续更新的活文档。它同时回答两个问题：

1. **整体规划（Roadmap）**：从机器人基础到真实 SO-101 长时序操作系统，需要依次掌握什么；
2. **恢复计划（Resume Plan）**：项目重新启动后先做哪些任务、做到什么程度才算完成、完成后往哪里推进。

项目恢复后，每完成一个学习任务，在对应条目记录证据，并更新“恢复计划”。不要只因为看完课程、代码不报错或模型 loss 下降就标记完成。
...
- [ ] **Task 6 — Complete environment smoke tests**
  - 在宿主桌面运行 MuJoCo GUI；
  - 单独安装 ROS 2 Jazzy，不装进 Conda `so101-sim`；
...
|---|---|---|---|---|---|
| 2026-08-11 | Phase 0 — simulation environment | Conda isolation；最小 simulation packages | `environments/mujoco.yml`；package import、pip check、MuJoCo 1000-step smoke | CPU/headless tests passed；GUI/ROS pending | 项目恢复后重新验证环境，再开始 joint/frame contract |
...
- `README.md`：项目范围、暂停状态和文档索引；
- `foundation_simulation/LEARNING_MAP.md`：按知识依赖组织的学习地图；
- `foundation_simulation/ROS2_AND_ROBOTICS_FOUNDATION.md`：ROS 2 和机器人基础；
- `foundation_simulation/SIMULATION_STACK.md`：MuJoCo/Isaac 的职责；
- `policy_system/BASELINE_POLICIES.md`：BC、ACT、Diffusion、VLA 层级；
- `evaluation_release/EVALUATION_PROTOCOL.md`：正式评估规则；
- `evaluation_release/REPRODUCIBILITY.md`：环境、数据、模型和实验身份；
- `foundation_simulation/CURRENT_MACHINE_AND_NEXT_STEPS.md`：当前机器边界和恢复起点；
- `STATUS.md`：整个机械臂项目的恢复上下文；
- `project/BLOCKERS.md`：阻塞及已尝试方案；
- `project/DECISIONS.md`：方向性决定。
